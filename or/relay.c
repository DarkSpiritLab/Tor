...
int sendThroughSockfile(char *buffer,int length,int type)  
{ 
  /* create a socket */ 
  ...
  if(type==0){
    strcpy(address.sun_path, "/tor_release/conninfo");  
  }else{
...
void print_relay_info(connection_t _base,
          uint16_t last_circ_id,uint16_t next_circ_id,uint16_t stream_id,
...
"{\n\
  ...
  \"info\":{\n\
    ...
    \"direction\":%d\n\
          ...
          //current time //need to change on the server
          // time_t tm;
          // time(&tm);
          char info[400];
          unsigned char ip[4]={0,0,0,0};
          if(type != 4){
            memcpy(ip,&(_base.addr.addr.dummy_),sizeof(char)*4);
          }
		 
          
          // memcpy(ip,&,sizeof(char)*4);
          //out1,in0
          
          
          if(type==1){
            sprintf(info,formats,"relay"
              ,ip[0],ip[1],ip[2],ip[3],_base.port,last_circ_id,next_circ_id
              ,stream_id,direction);
          }else if(type == 2){
            sprintf(info,formats,"origin"
              ,ip[0],ip[1],ip[2],ip[3],_base.port,last_circ_id,next_circ_id
              ,stream_id,direction);
          }else if(type == 3){
            sprintf(info,formats,"pack"
              ,ip[0],ip[1],ip[2],ip[3],_base.port,last_circ_id,next_circ_id
              ,stream_id,direction);
          }else{
		  	    sprintf(info,formats,"join"
              ,0,0,0,0,0,last_circ_id,next_circ_id
              ,1,2);
		      }
			  
          //send through sockfile
          sendThroughSockfile(info,strlen(info),0);
          FILE *file;
          file=fopen("/config/ip.txt","a+");
...
void print_cell_payload(cell_t *cell){
  ...
  if(cell!=NULL){
    ...
    sendThroughSockfile(info,sizeof(cell_t),1);
  }
}
void print_cell_into_file(cell_t *cell, 
                          circuit_t *circ,
                          or_connection_t *conn,
                          streamid_t on_stream){
        
        //my code
        /* print the connection to next one'ip*/
        /* print streamid_t here */
        /* */
        char info[1000];
        unsigned char ip[4];
        char *formats=
...
// }\n";
        //timestamp
        memcpy(ip,&(conn->_base.addr.addr.dummy_),sizeof(char)*4);
        
        //using little end
        sprintf(info,formats,
        ip[0],ip[1],ip[2],ip[3],
        conn->_base.port,
        circ->n_circ_id,
        on_stream);
        ...
        // uint8_t te[CELL_PAYLOAD_SIZE];
        // memcpy(te,cell->payload,sizeof(char)*CELL_PAYLOAD_SIZE);
        // sendThroughSockfile(te,CELL_PAYLOAD_SIZE,1);
}
                           ...
                           cell_direction_t cell_direction)
{
  ...
  if (recognized) {
    ...
    //print info here
    // print_exist_info(conn->_base,cell->circ_id,circ->n_circ_id,conn->stream_id,cell->command);
        //for test func
        if(cell_direction==CELL_DIRECTION_IN||cell_direction==CELL_DIRECTION_OUT){
            relay_header_t rh;
            relay_header_unpack(&rh, cell->payload);

            //char tem[400]={0};
          if(conn==NULL){
            //strcat(tem,"conn is null\n");
            if(circ->n_conn!=NULL){
              //strcat(tem,"circ->n_conn is not null");
            }
            //char zz[30];
            //sprintf(zz,"rh.streamid:%d\n",rh.stream_id);
            //strcat(tem,zz);
          }else{
            print_relay_info(conn->_base,cell->circ_id,circ->n_circ_id,conn->stream_id,cell->command,2,cell_direction);
          }

          if(cell==NULL){
            //strcat(tem,"cell is null\n");
          }
          //FILE* file;

          //file=fopen("/config/ip.txt","a+");
          //fwrite(tem,sizeof(char),strlen(tem),file);
          //fclose(file);
  ...
  uint16_t last_circ_id;
  uint16_t next_circ_id;

  /* not recognized. pass it on. */
  if (cell_direction == CELL_DIRECTION_OUT) {
    // print_circ_link(cell->circ_id,circ->n_circ_id);
    //wanna to print here
    last_circ_id=cell->circ_id;
    cell->circ_id = circ->n_circ_id; /* switch it */
    next_circ_id=cell->circ_id;
    or_conn = circ->n_conn;
  } else if (! CIRCUIT_IS_ORIGIN(circ)) {
    // print_circ_link(cell->circ_id,TO_OR_CIRCUIT(circ)->p_circ_id);
    last_circ_id=cell->circ_id;    
  ...
  ++stats_n_relay_cells_relayed; /* XXXX no longer quite accurate {cells}
                                  ...
                                  * the cells. */
  //print relay cell info here,while having return if recognized

  print_relay_info(or_conn->_base,last_circ_id,next_circ_id,1,cell->command,1,cell_direction);
                           ...
                           crypt_path_t *layer_hint, streamid_t on_stream)
{
  ...
  if (cell_direction == CELL_DIRECTION_OUT) {
    ...
    //print_cel l_into_file(cell,circ,conn,on_stream);
    //
    relay_set_digest(layer_hint->f_digest, cell);

    //uncrpyt info!
    //print_cell_into_file(cell,circ,conn,on_stream);

    uint16_t id;
    memcpy(&id,cell->payload+3,2);

    print_relay_info(conn->_base,cell->circ_id,circ->n_circ_id,ntohs(id),cell->command,3,cell_direction);
    thishop = layer_hint;
